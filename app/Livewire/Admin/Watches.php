    ...
    public function render()
    {
        ...
        $products = Product::join('categories', 'products.category_id', '=', 'categories.id')
            ->select(
                ...
                'products.machine_type',
                'products.overlock_stitch',
                'products.max_sewing_speed',
                'products.max_sewing_thickness',
                'products.color',
                'products.size',
                'products.weight',
                'products.origin',
                'products.transport_package',
                'products.description',
                'products.image_urls',
                'products.status',
                'categories.name as category_name',
            )
            ->where('products.model', 'like', '%' . $this->search . '%')
            ->orWhere('products.code', 'like', '%' . $this->search . '%')
            ->orWhere('products.brand', 'like', '%' . $this->search . '%')
            ->orWhere('products.status', 'like', '%' . $this->search . '%')
            ->orderBy('products.created_at', 'desc')
            ->paginate(10);

        // Dropdowns / filter data (example)
